...
export const EXTRACT_SUBAGENT_SYSTEM = [
  ...
  "- Prefer grep over readLines for exploration.",
  "- Hard limit: readLines can be called at most 2 times consecutively; you must call grep before calling readLines again.",
  "- Hard limit: do not call readLines with a previously used start/end range.",
  "- The first preview lines can be navigation, ads, login walls, or other boilerplate. Do not conclude from the preview alone.",
  "- If the first visible lines are low-signal, use grep/readLines to locate the real content sections before deciding.",
  "- Do not call writeExtractResult too early. Gather enough evidence first, then call writeExtractResult once at the very end.",
